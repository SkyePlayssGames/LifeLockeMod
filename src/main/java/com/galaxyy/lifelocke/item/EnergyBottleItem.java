...
public class EnergyBottleItem extends Item {
    ...
    public static class EnergyBottleGroup {
        public final EnergyBottleItem regular;
        public final EnergyBottleItem extended;
        public final EnergyBottleItem burst;

        public EnergyBottleGroup(
                Properties settings, Holder<MobEffect> type, Identifier id) {

            Identifier regular_id = Identifier.fromNamespaceAndPath(id.getNamespace(), id.getPath() + "_bottle");
            Identifier burst_id = Identifier.fromNamespaceAndPath(id.getNamespace(), "burst_" + id.getPath() + "_bottle");
            Identifier extended_id = Identifier.fromNamespaceAndPath(id.getNamespace(), "long_" + id.getPath() + "_bottle");

            regular = new EnergyBottleItem(settings.setId(
                    ResourceKey.create(Registries.ITEM, regular_id)), type, EffectTime.HALF_AN_HOUR);

            burst = new EnergyBottleItem(settings.setId(
                    ResourceKey.create(Registries.ITEM, burst_id)), type, EffectTime.MINUTE);

            extended = new EnergyBottleItem(settings.setId(
